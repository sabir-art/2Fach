...
(() => {
  ...
  // Unsplash asks for a referral link back to the photographer's source;
  // the utm_source identifies this site as the referrer.
  const UNSPLASH_HOMEPAGE_HREF =
    'https://unsplash.com/?utm_source=zweifach&utm_medium=referral';
  ...
  const withReferral = (href) => {
    try {
      ...
      if (!u.searchParams.has('utm_source')) {
        u.searchParams.set('utm_source', 'zweifach');
      }
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // (it is authored into the HTML) so it passes through unchanged.
